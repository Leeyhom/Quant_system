# -*- coding: utf-8 -*-
"""JoinQuant A-share simulation strategy v10.

v10 keeps the v9 alpha and adds one live-start risk control.

Why v10 exists:
  - v9 is the best full-window JoinQuant result so far, but a 2025-01-01
    cold-start backtest can be negative because the strategy is stateful:
    carried positions, 60-day rebalance phase, and the holding buffer depend
    on the start date.
  - A paper/live account also starts from cash, so this is not just a cosmetic
    backtest issue. The first few rebalances are the fragile part.
  - v10 therefore ramps exposure over the first three rebalances:
    70% -> 85% -> 98%. This is not a new alpha source; it is startup path
    control for simulation/live deployment.

v9 baseline that v10 preserves:
  - value_blend: 1/PE, 1/PCF, 1/PS first become one value signal, so value
    no longer votes three times in the final score;
  - orthogonal signals: growth_peg, amihud, quality_roe, low_vol_60;
  - 120-day momentum filter avoids deeply falling value traps;
  - holder is explicitly disabled because v6/v7 JoinQuant logs show
    holder_coverage=0. The strategy should not claim an unavailable signal;
  - v9 default: raise target exposure from 95% to 98% to reduce cash drag from
    100-share lot constraints;
  - v9 default: turnover buffer keeps existing names that are still in the
    top 1.5 * TOP_N score band, reducing unnecessary churn;
  - research toggle for bounded volatility budget is kept in the file, but
    disabled by default because local validation showed it reduced return and
    Sharpe on the current cached pool.

Local validation on cached DEFAULT_POOL, 2019-2025, 60k CNY lot/cost model:
  no-holder v6 shape / top10 / rebalance60 / industry_cap2 / mom120>-10%
  total return +257%, annualized +20.8%, Sharpe 0.99, max drawdown 18.1%.

JoinQuant v6 actual export, same executable factor set:
  total return +188%, annualized +17.0%, raw Sharpe 0.96, max drawdown 22.3%.

Paste this file into JoinQuant strategy editor. Set initial capital to
60000 CNY. If you want a pure historical benchmark with no startup guard, keep
using v9; if you are starting a new simulation/paper/live account, v10 is the
safer candidate.
"""
import math
...
STRATEGY_VERSION = "jq-cn-sim-v10-startup-ramp"
...
def initialize(context):
    ...
    g.rebalance_count = 0
    g.current_rebalance_no = 0
    g.current_target_exposure = STARTUP_EXPOSURES[0] if STARTUP_RAMP_ENABLED else MAX_EXPOSURE
    ...
    send_feishu(
        ...
        "版本: {}\n"
        "本金: 60000\n"
        "配置: {} 只 / 单行业最多 {} 只 / {} 交易日再平衡 / 满仓目标 {:.0f}% / 120日动量>{:.0f}%\n"
        "执行: 持仓缓冲 rank<={:.1f}xTOP_N / 有界低波预算={} / 单票上限 {:.1f}%\n"
        "启动保护: {} {}\n"
        "因子: {} ({}因子)\n"
        "说明: v10不改v9因子，只在新账户启动的前三次调仓逐步加仓，降低冷启动路径风险 ({})".format(
            STRATEGY_VERSION,
            TOP_N, INDUSTRY_CAP, REBALANCE_DAYS, MAX_EXPOSURE * 100, MOMENTUM_120_MIN * 100,
            HOLD_MULTIPLIER, USE_VOL_BUDGET, MAX_SINGLE_WEIGHT * 100,
            "开启" if STARTUP_RAMP_ENABLED else "关闭",
            "->".join("{:.0f}%".format(x * 100) for x in STARTUP_EXPOSURES) if STARTUP_RAMP_ENABLED else "",
...
def rebalance(context):
    ...
    g.current_rebalance_no = getattr(g, "rebalance_count", 0) + 1
    g.current_target_exposure = target_exposure_for_current_rebalance()

    candidates, blocked = filter_universe(STOCK_POOL)
    scores = build_scores_v10(context, candidates)
    ...
    log.info(
        "v10 rebalance {} no={} target_exposure={:.2f} factors={} candidates={} scored={} holder_coverage={} targets={}".format(
            today,
            g.current_rebalance_no,
    ...
    send_feishu(msg)
    g.rebalance_count = getattr(g, "rebalance_count", 0) + 1


def build_scores_v10(context, stocks):
    """Build v10 scores: unchanged v9 factor score plus cached vol."""
    ...
    score = score.dropna()
    log.info(
        "v10 factor columns before scoring: {} holder_coverage={}".format(
...
def select_targets(context, scores, top_n=TOP_N, industry_cap=INDUSTRY_CAP):
    ...
    def eligible(stock):
        ...
        if pd.notna(mom120) and mom120 <= MOMENTUM_120_MIN:
            ...
            return False
        slot_value = portfolio_value * target_exposure / top_n
...
def build_target_weights(selected):
    """Convert selected names into target weights.

    The risk-budget tilt is intentionally bounded. v7 showed that aggressive
    score sizing can amplify path risk under small-capital lot constraints; v9
    only lets volatility change weights by a narrow band around equal weight.
    """
    if not selected:
        return pd.Series(dtype=float)
    target_exposure = current_target_exposure()
    if not USE_VOL_BUDGET or len(selected) < 3:
        return pd.Series(target_exposure / len(selected), index=selected)
    ...
    weights = raw / raw.sum() * target_exposure

    avg = target_exposure / len(selected)
    ...
    return weights / weights.sum() * target_exposure


def target_exposure_for_current_rebalance():
    """Target exposure for this rebalance, before incrementing rebalance_count."""
    if not STARTUP_RAMP_ENABLED:
        return MAX_EXPOSURE
    if not STARTUP_EXPOSURES:
        return MAX_EXPOSURE
    idx = min(getattr(g, "rebalance_count", 0), len(STARTUP_EXPOSURES) - 1)
    return min(MAX_EXPOSURE, float(STARTUP_EXPOSURES[idx]))


def current_target_exposure():
    """Current target exposure set by rebalance(); falls back to full v9 target."""
    return float(getattr(g, "current_target_exposure", MAX_EXPOSURE))
...
def format_rebalance_message(context, targets, weights, actions, skipped, scores):
    ...
    return (
        ...
        "因子: {}\n"
        "holder覆盖: {}\n"
        "目标仓位: {:.0f}% / 满仓目标 {:.0f}% / 第{}次调仓 / {} 只 / 持仓缓冲 {:.1f} / 波动预算 {}\n\n"
        "目标持仓:\n{}\n\n"
        "交易:\n{}\n\n"
        "跳过/失败:\n{}"
    ).format(
        ...
        target_exposure * 100,
        MAX_EXPOSURE * 100,
        getattr(g, "current_rebalance_no", 0),
        len(targets),
        HOLD_MULTIPLIER,
        USE_VOL_BUDGET,
