# -*- coding: utf-8 -*-
"""JoinQuant A-share simulation strategy v9.

v9 is a conservative improvement on the v8 recovery baseline:
  ...
  - NEW default: raise target exposure from 95% to 98% to reduce cash drag from
    100-share lot constraints;
  - NEW default: turnover buffer keeps existing names that are still in the
...
STRATEGY_VERSION = "jq-cn-sim-v9-less-cash-buffer"
...
def initialize(context):
    ...
    send_feishu(
        ...
        "因子: {} ({}因子)\n"
        "说明: v8稳健恢复版的组合层增强：不改因子，提高目标仓位到98%并保留仍靠前的老持仓 ({})".format(
            STRATEGY_VERSION,
...
def rebalance(context):
    ...
    scores = build_scores_v9(context, candidates)
    ...
    log.info(
        "v9 rebalance {} factors={} candidates={} scored={} holder_coverage={} targets={}".format(
            today,
...
def build_scores_v9(context, stocks):
    """Build v9 scores: v8 factor score plus cached vol for risk budgeting."""
    ...
    score = score.dropna()
    log.info(
        "v9 factor columns before scoring: {} holder_coverage={}".format(
