# -*- coding: utf-8 -*-
"""JoinQuant A-share CN800 v5 — 当前生产基线（v2~v8迭代中最优）。

v5 = v4 + 波动率目标降仓 + ROE稳定性因子（7因子体系）。
...
组合优化:
  ...
  - 波动率目标降仓: 25%目标, 50%底仓（高波动期降风险）
  - 动态行业上限: max(2, 池内行业股票数/10)
  - 资金效率: MAX_SLOT_MULT 1.50

本地 walk-forward（v3因子集, train=480/test=120/step=120, 11窗口）验证了池子扩大的收益:
  CN800池(702只): +182.8%, 年化+22.0%, Sharpe 0.98, 回撤 34.8%
  旧池(91只): +60.3%, 年化+9.4%, Sharpe 0.48, 回撤 46.2%
  → 不改因子仅扩池，收益+122pct, Sharpe翻倍

诚实声明:
  ① 因子构造只用prev_date（历史数据），无未来函数
  ② 波动率目标使用trailing窗口，天然不含当日数据
  ③ Walk-forward 验证 OOS Sharpe 0.98, 非全样本回测

回测设置: 本金 60000, 区间 2019-01-01 ~ 2025-12-31, 基准 沪深300
...
MOMENTUM_BOOST_WEIGHT   =  0.3
QUALITY_WEIGHT = 0.5
LOWVOL_WEIGHT = 0.5
RESIDUAL_MOM_WEIGHT = 0.5
# Phase 3: 波动率目标降仓（CN A股最优25%, 只降仓不加杠杆）
VOL_TARGET = 0.25
VOL_LOOKBACK = 60
VOL_FLOOR = 0.50
# Phase 4: ROE稳定性因子（区分"真便宜"与"价值陷阱"）
ROE_STABILITY_WEIGHT = 0.3
ROE_STABILITY_WINDOW = 120
HOLDER_WEIGHT = 0.0
INCLUDE_HOLDER = False
FACTOR_NAMES = [
    "value_blend", "growth_peg", "amihud", "quality_roe", "low_vol_60",
    "residual_momentum", "roe_stability",
...
# Phase 1: 资金效率优化
MAX_SLOT_MULT = 1.50
STRATEGY_VERSION = "jq-cn800-v5-voltarget-roe-stability"
...
def initialize(context):
    ...
    # Phase 3: 每日总资产追踪（用于波动率目标计算）
    g.portfolio_values = []

    # Phase 4: ROE历史追踪（每只股票最近120天PB/PE值）
    g.roe_history = {}

    run_daily(rebalance, time="14:30")

    send_feishu(
        "A股聚宽模拟盘 CN800 v5 已启动\n"
        "版本: {}\n"
        "本金: 60000\n"
        "池子: {} 只 (沪深300+中证500, SW{}行业)\n"
        "配置: top{} / 行业cap动态(max{},N/{}) / {}日再平衡 / {:.0f}%仓位\n"
        "      自适应动量(牛市{:.0f}%/熊市{:.0f}%/默认{:.0f}%) / 资金乘数{:.1f}x\n"
        "      vol目标{:.0f}%(底仓{:.0f}%) / ROE稳定性(权重{},窗口{}d)\n"
        "因子: {} ({}个)\n"
        "v5新增: 波动率目标降仓 + ROE稳定性因子".format(
            STRATEGY_VERSION, len(STOCK_POOL), len(INDUSTRY_MAP),
            TOP_N, INDUSTRY_CAP_BASE, INDUSTRY_CAP_RATIO,
            REBALANCE_DAYS, MAX_EXPOSURE * 100,
            MOMENTUM_120_MIN_BULL * 100, MOMENTUM_120_MIN_BEAR * 100,
            MOMENTUM_120_MIN_DEFAULT * 100, MAX_SLOT_MULT,
            VOL_TARGET * 100, VOL_FLOOR * 100,
            ROE_STABILITY_WEIGHT, ROE_STABILITY_WINDOW,
...
def _compute_effective_exposure():
    ...
    从 g.portfolio_values 计算 trailing 60 日年化波动率。
    如果波动率 > VOL_TARGET(25%)：按比例降仓，最低不低于 VOL_FLOOR(50%)。
    如果波动率 <= 目标：保持 MAX_EXPOSURE(98%)满仓。

    防未来函数：trailing window 使用「今天之前」的资产序列，
    不包含当天尚未发生的价格变化。
    """
    vals = getattr(g, "portfolio_values", [])
    if len(vals) < VOL_LOOKBACK:
        return MAX_EXPOSURE

    series = pd.Series(vals)
    daily_ret = series.pct_change().dropna().tail(VOL_LOOKBACK)
    if len(daily_ret) < 20:
        return MAX_EXPOSURE

    trailing_vol = float(daily_ret.std() * np.sqrt(252))
    if trailing_vol > VOL_TARGET and trailing_vol > 0:
        scaled = MAX_EXPOSURE * VOL_TARGET / trailing_vol
        return max(VOL_FLOOR, scaled)
    return MAX_EXPOSURE
...
def rebalance(context):
    ...
    # Phase 3: 每日记录组合总资产（用于波动率计算）
    g.portfolio_values.append(context.portfolio.total_value)
    if len(g.portfolio_values) > VOL_LOOKBACK + 5:
        g.portfolio_values.pop(0)

    # 计算当前有效敞口（波动率目标降仓）
    g.current_effective_exposure = _compute_effective_exposure()
    ...
    log.info(
        "cn800 v5 rebalance {} regime={} mom={:.0%} eff_exp={:.0%} factors={} candidates={} scored={} targets={}".format(
            today,
            getattr(g, "market_regime", "neutral"),
            getattr(g, "momentum_filter", MOMENTUM_120_MIN_DEFAULT),
            getattr(g, "current_effective_exposure", MAX_EXPOSURE),
...
def build_scores(context, stocks):
    ...
    roe_today = (pb.replace(0, np.nan) / pe.replace(0, np.nan)).where((pe > 0) & (pb > 0))
    factors["quality_roe"] = roe_today

    # Phase 4: ROE稳定性 = -CV(PB/PE, 120d)（越稳定分越高）
    # 更新每只股票的ROE历史（dict of list，最近120天）
    for stock in roe_today.index:
        val_roe = roe_today.get(stock, np.nan)
        if pd.notna(val_roe):
            if stock not in g.roe_history:
                g.roe_history[stock] = []
            g.roe_history[stock].append(float(val_roe))
            if len(g.roe_history[stock]) > ROE_STABILITY_WINDOW:
                g.roe_history[stock].pop(0)

    # 对历史≥60天的股票计算ROE稳定性
    roe_stability = pd.Series(np.nan, index=roe_today.index)
    for stock, hist in g.roe_history.items():
        if len(hist) >= 60 and stock in roe_stability.index:
            s = pd.Series(hist)
            cv = s.std() / s.mean() if s.mean() > 0 else np.nan
            roe_stability[stock] = -cv  # 取负：越稳定分越高
    # 只在覆盖足够时添加（首年预热期自动跳过）
    if roe_stability.notna().sum() >= 20:
        factors["roe_stability"] = roe_stability
    ...
    # 120日动量 + 市场状态检测（Phase 2: 自适应过滤）
    if len(close) >= 121:
        g.last_momentum_120 = close.iloc[-1] / close.iloc[-121].replace(0, np.nan) - 1.0
        # 用全池等权平均120日收益近似市场动量（替代沪深300基准）
        market_mom = g.last_momentum_120.mean()
        if pd.notna(market_mom):
            if market_mom > MOMENTUM_MARKET_BULL:
                g.market_regime = "bull"
                g.momentum_filter = MOMENTUM_120_MIN_BULL
            elif market_mom < MOMENTUM_MARKET_BEAR:
                g.market_regime = "bear"
                g.momentum_filter = MOMENTUM_120_MIN_BEAR
            else:
                g.market_regime = "neutral"
                g.momentum_filter = MOMENTUM_120_MIN_DEFAULT
        else:
            g.market_regime = "neutral"
            g.momentum_filter = MOMENTUM_120_MIN_DEFAULT
        # 牛市中：正动量股票获得额外加分（对抗价值陷阱）
        if g.market_regime == "bull":
            factors["raw_momentum_120"] = g.last_momentum_120
    else:
        g.last_momentum_120 = pd.Series(dtype=float)
        g.market_regime = "neutral"
        g.momentum_filter = MOMENTUM_120_MIN_DEFAULT
    ...
    #           + quality_roe(0.5) + low_vol_60(0.5) + residual_momentum(0.5)
    #           + raw_momentum_120(0.3, 仅牛市)  [Phase 2: 动量增强]
    ...
    if "raw_momentum_120" in weights.index:
        weights["raw_momentum_120"] = MOMENTUM_BOOST_WEIGHT
    if "roe_stability" in weights.index:
        weights["roe_stability"] = ROE_STABILITY_WEIGHT
...
def select_targets(context, scores, top_n=TOP_N, industry_cap_base=INDUSTRY_CAP_BASE):
    ...
    eff_exp = getattr(g, "current_effective_exposure", MAX_EXPOSURE)
    slot_value = portfolio_value * eff_exp / top_n
    mom = getattr(g, "last_momentum_120", pd.Series(dtype=float))
    # Phase 2: 自适应动量阈值（市场状态驱动）
    mom_filter = getattr(g, "momentum_filter", MOMENTUM_120_MIN_DEFAULT)
    market_regime = getattr(g, "market_regime", "neutral")
    ...
    def eligible(stock):
        ...
        if pd.notna(mom120) and mom120 <= mom_filter:
            return False
        # Phase 1: 放宽资金约束（1.50x替代1.15x，减少现金闲置）
        if price * 100 > slot_value * MAX_SLOT_MULT:
            return False
        return True
...
def build_target_weights(selected):
    if not selected:
        return pd.Series(dtype=float)
    target_exp = getattr(g, "current_effective_exposure", MAX_EXPOSURE)
    if not USE_VOL_BUDGET or len(selected) < 3:
        return pd.Series(target_exp / len(selected), index=selected)
    ...
    weights = raw / raw.sum() * target_exp

    avg = target_exp / len(selected)
    ...
    return weights / weights.sum() * target_exp
...
def format_rebalance_message(context, targets, weights, actions, skipped, scores):
    ...
    return (
        ...
        getattr(g, "current_effective_exposure", MAX_EXPOSURE) * 100,
        MAX_EXPOSURE * 100,
        len(targets),
        HOLD_MULTIPLIER,
        USE_VOL_BUDGET,
