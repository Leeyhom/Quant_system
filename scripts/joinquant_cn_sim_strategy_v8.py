# -*- coding: utf-8 -*-
"""JoinQuant A-share simulation strategy v8.

v8 is the recovery/robustness successor after the v7 failure:
...
STRATEGY_VERSION = "jq-cn-sim-v8-robust-recovery"
...
def initialize(context):
    ...
    send_feishu(
        ...
        "因子: {} ({}因子)\n"
        "说明: v7失败后的稳健恢复版：value_blend + 质量低波半权重 + 趋势过滤 + 等权 ({})".format(
            STRATEGY_VERSION,
...
def rebalance(context):
    ...
    scores = build_scores_v8(context, candidates)
    ...
    log.info(
        "v8 rebalance {} factors={} candidates={} scored={} holder_coverage={} targets={}".format(
            today,
...
def build_scores_v8(context, stocks):
    """Build v8 scores: value_blend + growth/liquidity/quality/low-vol."""
    ...
    score = score.dropna()
    log.info(
        "v8 factor columns before scoring: {} holder_coverage={}".format(
