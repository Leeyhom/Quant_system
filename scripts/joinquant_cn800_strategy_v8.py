...
STRATEGY_VERSION = "jq-cn800-v8-st-reversal"
...
def initialize(context):
    ...
    send_feishu(
        ...
        "因子: {} ({}个)\n"
        "v8新增: 短期反转因子(st_reversal,权重{:.1f})".format(
...
def rebalance(context):
    ...
    log.info(
        "cn800 v8 rebalance {} regime={} mom={:.0%} factors={} candidates={} scored={} targets={}".format(
            today,
...
def build_scores(context, stocks):
    ...
    # v8新增: 短期反转 = -ret_20d（过去20日收益取负，跌多了分高）
    # A股散户主导→短期过度反应→随后均值回归。与60日残差动量负相关，信息互补
    if len(close) >= 21:
        factors["st_reversal"] = -close.pct_change(20, fill_method=None).iloc[-1]
    ...
    if "st_reversal" in weights.index:
        weights["st_reversal"] = ST_REVERSAL_WEIGHT
