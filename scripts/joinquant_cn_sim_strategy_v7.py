# -*- coding: utf-8 -*-
"""JoinQuant A-share simulation strategy v7.

v7 is the production-oriented successor of v6:
  ...
  - holder is explicitly disabled because v6 JoinQuant logs showed
    holder_coverage=0, so the deployed strategy was not actually using it;
  - rebalance every 50 trading days and use score-tilted position sizing:
    higher-score names get modestly larger weights while single-name exposure
    remains capped;
...
Local validation on cached DEFAULT_POOL, 2019-2025, 60k CNY lot/cost model:
  q05_lv05 / top10 / rebalance50 / industry_cap2 / no momentum filter /
  fixed95 / tilt065
  total return +358%, annualized +25.3%, Sharpe 1.20, max drawdown 20.2%.
...
SCORE_TILT = 0.65
MIN_WEIGHT_MULTIPLIER = 0.58
MAX_WEIGHT_MULTIPLIER = 1.45
MAX_SINGLE_WEIGHT = 0.145
FACTOR_NAMES = [
    "value_blend", "growth_peg", "amihud", "quality_roe", "low_vol_60",
] + (["holder_concentration"] if INCLUDE_HOLDER else [])
STRATEGY_VERSION = "jq-cn-sim-v7-score-tilt"
...
def initialize(context):
    ...
    _momentum_note = "120日动量>{:.0f}%".format(MOMENTUM_120_MIN * 100) if USE_MOMENTUM_FILTER else "不做个股动量过滤"
    send_feishu(
        "A股聚宽模拟盘 v7 已启动\n"
        "版本: {}\n"
        "本金: 60000\n"
        ...
        "因子: {} ({}因子)\n"
        "说明: value_blend降维价值因子 + 质量低波半权重 + 分数倾斜配仓 ({}, tilt={:.2f})".format(
            STRATEGY_VERSION,
            TOP_N, INDUSTRY_CAP, REBALANCE_DAYS, MAX_EXPOSURE * 100, _momentum_note,
            "+".join(FACTOR_NAMES), len(FACTOR_NAMES),
            _holder_note, SCORE_TILT,
        )
    )
...
def rebalance(context):
    ...
    scores = build_scores_v7(context, candidates)
    ...
    log.info(
        "v7 rebalance {} factors={} candidates={} scored={} holder_coverage={} targets={}".format(
            today,
...
def build_scores_v7(context, stocks):
    """Build v7 scores: value_blend + growth/liquidity/quality/low-vol."""
    ...
    score = score.dropna()
    log.info(
        "v7 factor columns before scoring: {} holder_coverage={}".format(
...
def select_targets(context, scores, top_n=TOP_N, industry_cap=INDUSTRY_CAP):
    ...
    for stock in scores.sort_values(ascending=False).index:
        ...
        if USE_MOMENTUM_FILTER and pd.notna(mom120) and mom120 <= MOMENTUM_120_MIN:
        ...
        if len(selected) >= top_n:
            break
    targets = score_tilt_weights(selected, scores, MAX_EXPOSURE)
    return targets, skipped


def score_tilt_weights(selected, scores, exposure):
    """Turn selected names into score-tilted weights with a single-name cap."""
    if not selected:
        return {}
    selected_scores = scores.reindex(selected)
    rank = selected_scores.rank(pct=True)
    raw = 1.0 + SCORE_TILT * (rank - rank.mean())
    raw = raw.clip(lower=0.45, upper=1.55)
    weights = raw / raw.sum() * exposure

    avg = exposure / len(selected)
    weights = weights.clip(
        lower=avg * MIN_WEIGHT_MULTIPLIER,
        upper=min(MAX_SINGLE_WEIGHT, avg * MAX_WEIGHT_MULTIPLIER),
    )
    weights = weights / weights.sum() * exposure
    return {stock: float(weights.loc[stock]) for stock in selected}
