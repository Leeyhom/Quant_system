...
VM_INTERACTION_WEIGHT = 0.3        # v7新增: 价值×动量交互项权重
HOLDER_WEIGHT = 0.0
INCLUDE_HOLDER = False
FACTOR_NAMES = [
    "value_blend", "growth_peg", "amihud", "quality_roe", "low_vol_60",
    "residual_momentum", "vm_interaction",
...
STRATEGY_VERSION = "jq-cn800-v7-vm-interaction"
...
def initialize(context):
    ...
    send_feishu(
        ...
        "因子: {} ({}个)\n"
        "v7新增: 价值×动量交互项(vm_interaction,权重{:.1f})".format(
...
def rebalance(context):
    ...
    log.info(
        "cn800 v7 rebalance {} regime={} mom={:.0%} factors={} candidates={} scored={} targets={}".format(
            today,
...
def build_scores(context, stocks):
    ...
    # v7: 价值×动量交互项 — rank(value) × rank(动量)
    # 便宜+正动量 → 高分；便宜+负动量 → 被惩罚（价值陷阱折价）
    if "value_blend" in frame.columns and "residual_momentum" in frame.columns:
        rv = frame["value_blend"].rank(pct=True)
        rm = frame["residual_momentum"].rank(pct=True)
        frame["vm_interaction"] = rv * rm
    ...
    if "vm_interaction" in weights.index:
        weights["vm_interaction"] = VM_INTERACTION_WEIGHT
