# -*- coding: utf-8 -*-
"""
JoinQuant A-share simulation strategy v5.
...
This is the non-simplified execution adapter for the local M14 A-share logic:
    ...
    - quality factor at half weight and a 120-day trend filter to reduce value traps
...
60000 CNY. The strategy rebalances at 14:30 every 60 trading days.
"""
...
REBALANCE_DAYS = 60
TRADE_TIME = "14:30"
STRATEGY_VERSION = "jq-cn-sim-v5-quality-trend"
...
def initialize(context):
    ...
    send_feishu(
        ...
        "版本: %s\n"
        "本金: 60000\n"
        "配置: 10只 / 单行业最多1只 / 60交易日再平衡 / 总仓位95%%\n"
        "风控: 质量因子半权重 + 120日动量不低于-10%%\n"
...
def rebalance(context):
    ...
    log.info(
        "v5 rebalance %s factors=%s candidates=%s scored=%s trend_filtered=%s targets=%s",
        today,
        ",".join(g.last_factor_names),
        g.last_candidate_count,
        g.last_score_count,
        g.last_trend_filtered,
        ",".join(targets),
    )
    send_feishu(msg)
...
def build_scores(context, stocks):
    ...
    if len(close) >= 121:
        ...
        before = len(frame)
        keep = mom120.reindex(frame.index)
        frame = frame.loc[(keep > MOMENTUM_120_MIN) | keep.isna()]
        g.last_trend_filtered = before - len(frame)
        if frame.empty:
            return pd.Series(dtype=float)

    frame = neutralize_cross_section(frame, mv)
    log.info(
        "v5 factor columns before scoring: %s trend_filtered=%s",
        ",".join(frame.columns),
        g.last_trend_filtered,
    )
    ranked = frame.rank(pct=True)
    weight = pd.Series(1.0, index=ranked.columns)
    if "quality_roe" in weight.index:
        weight["quality_roe"] = QUALITY_WEIGHT
    valid_weight = ranked.notna().mul(weight, axis=1).sum(axis=1)
    score = ranked.mul(weight, axis=1).sum(axis=1) / valid_weight.replace(0, np.nan)
    score = score.dropna()
