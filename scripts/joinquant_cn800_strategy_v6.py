...
MOMENTUM_120_MIN_DEFAULT = -0.10   # 震荡市默认
MOMENTUM_120_MIN_BULL   = -0.05    # 牛市中更严格：不持有任何显著下跌股
MOMENTUM_120_MIN_BEAR   = -0.25    # 熊市中更宽松：避免恐慌性清仓
MOMENTUM_MARKET_BULL    =  0.05    # 基准120日收益>+5% = 牛市
MOMENTUM_MARKET_BEAR    = -0.10    # 基准120日收益<-10% = 熊市
MOMENTUM_BOOST_WEIGHT   =  0.3     # 牛市中正动量额外加分权重
QUALITY_WEIGHT = 0.5
LOWVOL_WEIGHT = 0.5
RESIDUAL_MOM_WEIGHT = 0.7          # v6: 0.5→0.7, 动量信号更大话语权
# v6: 回撤刹车（只在真回撤时降仓，不误伤反弹——区别于v5的vol target）
DD_TRIGGER = 0.15                  # 净值从峰值回撤>15%触发降仓
DD_REDUCED = 0.60                  # 触发后保留60%仓位（~59%总敞口）
HOLDER_WEIGHT = 0.0
...
# Phase 1: 资金效率优化
MAX_SLOT_MULT = 1.50               # 放宽单票资金上限（原1.15，减少现金闲置）
STRATEGY_VERSION = "jq-cn800-v6-drawdown-brake"
...
def initialize(context):
    ...
    send_feishu(
        ...
        "v6新增: 回撤刹车(DDBrake) + 残差动量权重0.5→0.7".format(
            ...
            DD_TRIGGER * 100, DD_REDUCED * 100, RESIDUAL_MOM_WEIGHT,
...
def rebalance(context):
    ...
    # v6: 回撤刹车 — 只在真回撤时降仓，不误伤反弹
    total_val = context.portfolio.total_value
    g.peak_value = max(g.peak_value, total_val)
    dd = 1.0 - total_val / g.peak_value if g.peak_value > 0 else 0.0
    if dd > DD_TRIGGER:
        g.current_effective_exposure = MAX_EXPOSURE * DD_REDUCED
    else:
        g.current_effective_exposure = MAX_EXPOSURE
    ...
    dd_val = 1.0 - context.portfolio.total_value / g.peak_value if g.peak_value > 0 else 0.0
    log.info(
        "cn800 v6 rebalance {} regime={} mom={:.0%} dd={:.1%} eff={:.0%} factors={} candidates={} scored={} targets={}".format(
            today,
...
def format_rebalance_message(context, targets, weights, actions, skipped, scores):
    ...
    dd_val = 1.0 - context.portfolio.total_value / g.peak_value if g.peak_value > 0 else 0.0
    eff = getattr(g, "current_effective_exposure", MAX_EXPOSURE)
    return (
        "A股模拟盘调仓 (CN800 v6) {}\n"
        "总资产: {:.2f}\n"
        "现金: {:.2f}\n"
        "市场状态: {} (动量{:.0%}) 回撤: {:.1%} (峰值{:.0f})\n"
        "因子: {}\n"
        "有效仓位: {:.0f}% (满仓{:.0f}%) / {} 只 / 缓冲 {:.1f}\n\n"
        "目标持仓:\n{}\n\n"
        "交易:\n{}\n\n"
        "跳过/失败:\n{}"
    ).format(
