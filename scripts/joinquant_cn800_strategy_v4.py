...
STRATEGY_VERSION = "jq-cn800-v4-adaptive-momentum"
...
def initialize(context):
    ...
    send_feishu(
        "A股聚宽模拟盘 CN800 v4 已启动\n"
        "版本: {}\n"
        "本金: 60000\n"
        "池子: {} 只 (沪深300+中证500, SW{}行业)\n"
        "配置: top{} / 行业cap动态(max{},N/{}) / {}日再平衡 / {:.0f}%仓位\n"
        "      自适应动量过滤(牛市{:.0f}%/熊市{:.0f}%/默认{:.0f}%) / 资金乘数{:.1f}x\n"
        "因子: {} ({}个)\n"
        "v4新增: 市场状态自适应动量 + 牛市动量增强 + 放宽资金约束".format(
...
def rebalance(context):
    ...
    log.info(
        "cn800 v4 rebalance {} regime={} mom_filter={:.0%} factors={} candidates={} scored={} holder={} targets={}".format(
            today,
