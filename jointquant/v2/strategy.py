# -*- coding: utf-8 -*-
"""
JoinQuant A-share simulation strategy v2.
...
This is the non-simplified execution adapter for the local M14 A-share logic:
    ...
    - fixed 12-stock portfolio, one stock per industry, 95% max exposure
...
REBALANCE_DAYS = 60
TRADE_TIME = "14:30"
STRATEGY_VERSION = "jq-cn-sim-v2-full-factor"
...
def initialize(context):
    ...
    send_feishu(
        "A股聚宽模拟盘已启动\n" "版本: %s\n" "本金: 60000\n" "配置: 12只 / 单行业最多1只 / 60交易日再平衡 / 总仓位95%%\n" "执行: 14:30 自动调仓，未成交留现金" % STRATEGY_VERSION
    )
...
def rebalance(context):
    ...
    for stock in targets:
        ...
        actions.append("TARGET %s %.2f%%" % (stock, target_weight * 100))
